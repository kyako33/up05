...
class ResultPanel:
    def __init__(self, parent, is_success, level, on_catalog, on_retry=None, dark_theme=False):
        ...
        self.panel.place(relx=0.5, rely=0.5, anchor='center', width=700, height=600)
        ...
        for r, row in enumerate(self.pixel_canvases):
            for c, pixel in enumerate(row):
                if isinstance(pixel, int):
                    ...
                    self.image_canvas.itemconfig(pixel, fill=color)
    
    def _make_button(self, parent, text, cmd):
        # Стиль кнопки как в главном меню
    ...
    def show_success(self):
        ...
        # Поздравления с уменьшенными отступами
        tk.Label(self.panel, text="ПОЗДРАВЛЯЕМ!", font=('Courier', 32, 'bold'),
                bg=self.bg_color, fg='#4CAF50').pack(pady=(15, 5))
        
        tk.Label(self.panel, text=f"Вы собрали: {level.get('image_name', 'Картинка')}!", 
                font=('Courier', 24, 'bold'), bg=self.bg_color, fg="#D5EDED").pack(pady=(5, 5))
        
        tk.Label(self.panel, text=f"{level.get('name', '')} пройден!", 
                font=('Courier', 20, 'bold'), bg=self.bg_color, fg='#D9D9D9').pack(pady=(5, 15))
        ...
        if solution and rows > 0:
            image_frame = tk.Frame(self.panel, bg=self.bg_color)
            ...
            max_width = 500
            if cols * pixel_size > max_width:
                pixel_size = max_width // cols
            
            filled = '#D9D9D9'
            empty = '#14142B' if self.dark_theme else '#425B99'
            
            total_width = cols * pixel_size
            total_height = rows * pixel_size
            
            # Создаём canvas без сетки
            self.image_canvas = tk.Canvas(image_frame, width=total_width, height=total_height, 
                                         bg=self.bg_color, highlightthickness=0, bd=0)
            self.image_canvas.pack()
            
            # Рисуем клетки без обводки
            for r in range(min(rows, len(solution))):
                ...
                for c in range(min(cols, len(solution[r]) if solution else 0)):
                    ...
                    rect = self.image_canvas.create_rectangle(x1, y1, x2, y2, fill=color, outline='', width=0)
                    row_rects.append(rect)
                self.pixel_canvases.append(row_rects)
        
        # Кнопка в стиле главного меню
        self._make_button(self.panel, "К КАТАЛОГУ УРОВНЕЙ", self.close_and_go_to_catalog)
    
    def show_error(self):
        tk.Label(self.panel, text="ОШИБКА!", font=('Courier', 32, 'bold'),
                bg=self.bg_color, fg='#f44336').pack(pady=(25, 10))
        
        tk.Label(self.panel, text="Поле заполнено неверно!", font=('Courier', 22, 'bold'),
                bg=self.bg_color, fg='#f44336').pack(pady=(10, 10))
        
        tk.Label(self.panel, text="Попробуйте ещё раз", font=('Courier', 18, 'bold'),
                bg=self.bg_color, fg='#D9D9D9').pack(pady=(10, 20))
        
        self._make_button(self.panel, "ВЕРНУТЬСЯ К УРОВНЮ", self.close_and_retry)
        self._make_button(self.panel, "К КАТАЛОГУ УРОВНЕЙ", self.close_and_go_to_catalog)
